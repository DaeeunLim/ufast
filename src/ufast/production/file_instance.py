...
from ufast.production.classes import Machine, FileRoute, Lot
from ufast.production.events import BreakdownEvent
from ufast.production.instance import Instance
from ufast.production.randomizer import Randomizer
from ufast.production.tools import get_interval, get_distribution, UniformDistribution, date_time_parse


class FileInstance(Instance):

    def __init__(self, files: Dict[str, List[Dict]], run_to, lot_for_machine, plugins,
                 machine_equipment: Dict[str, List[Dict]] = None):
        self.run_to = float(run_to)
        machines = []
        machine_id = 0
        r = Randomizer()
        family_locations = {}
        machine_equipment = machine_equipment or {}
        for d_m in files['tool.txt.1l']:
            family = d_m['STNFAM']
            equipment_pool = machine_equipment.get(family, [])
            for i in range(int(d_m['STNQTY'])):
                speed = 1  # r.random.uniform(0.7, 1.3)
                m = Machine(idx=machine_id, d=d_m, speed=speed)
                if equipment_pool:
                    equipment = equipment_pool[i % len(equipment_pool)]
                    m.equipment_id = equipment.get('equipment_id')
                    m.node_name = equipment.get('node')
