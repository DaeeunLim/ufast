...
from ufast.production.classes import Lot, Machine
...
from ufast.production.stats import print_statistics
...
def dispatching_combined_permachine(ptuple_fcn, machine, time, setups):
    waiting_lots = machine.waiting_lots
    if getattr(machine, 'reserved_lots', None):
        reserved = set(machine.reserved_lots)
        waiting_lots = [lot for lot in machine.waiting_lots if lot in reserved]
...
def get_lots_to_dispatch_by_machine(instance, ptuple_fcn, machine=None):
    ...
    if getattr(machine, 'reserved_lots', None):
        reserved = set(machine.reserved_lots)
        waiting_lots = [lot for lot in machine.waiting_lots if lot in reserved]
    wl = sorted(waiting_lots, key=lambda k: k.ptuple)
    if not wl:
        return machine, None
...
def run_greedy():
    ...
    plugins = []
    if a.wandb:
        from ufast.production.plugins.wandb_plugin import WandBPlugin
        plugins.append(WandBPlugin())
    if a.chart:
        from ufast.production.plugins.chart_plugin import ChartPlugin
