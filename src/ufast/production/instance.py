...
from ufast.production.classes import Machine, Route, Lot
from ufast.production.dispatching.dm_lot_for_machine import LotForMachineDispatchManager
from ufast.production.dispatching.dm_machine_for_lot import MachineForLotDispatchManager
from ufast.production.event_queue import EventQueue
from ufast.production.events import MachineDoneEvent, LotDoneEvent, BreakdownEvent, ReleaseEvent
from ufast.production.plugins.interface import IPlugin
from ufast.common.equipment_kpi import close_starvation, record_equipment_downtime
...
class Instance:
        ...
        for lot in lots:
            ...
            while len(lot.remaining_steps) > 0:
                ...
                if lot.actual_step.has_to_perform():
                    ...
                    self._lot_ready_for_step(lot, old_step)
                    step_found = True
                    for plugin in self.plugins:
    ...
    def dispatch(self, machine: Machine, lots: List[Lot]):
        close_starvation(machine, self.current_time, lots)
        ...
        machine.busy_intervals.append((busy_start, busy_start + machine_time))
        ...
        for i in range(len(machine.pieces_until_maintenance)):
            ...
            if machine.pieces_until_maintenance[i] <= 0:
                ...
                record_equipment_downtime(self, machine, "pm", s)
    ...
    def reserve_machine_lot(self, lots, machine):
        ...
        if getattr(machine, 'reserved_lots', None):
            for lot in lots:
                if lot in machine.reserved_lots:
                    machine.reserved_lots.remove(lot)
                if getattr(lot, 'reserved_machine', None) is machine:
                    lot.reserved_machine = None
